...
export default function NewElectricalProjectPage() {
  const router = useRouter()
  const [error, setError] = useState('')

  const { register, handleSubmit, formState: { errors } } = useForm<FormData>({
    resolver: zodResolver(schema),
    defaultValues: { projectType: 'NEW_INSTALLATION', voltageLevel: 'LOW' },
  })

  const mutation = useMutation({
    mutationFn: (data: FormData) => api.post('/electrical', {
      ...data,
    ...
    onSuccess: () => router.push('/dashboard/electrical-projects'),
    onError: (err: any) => setError(err.response?.data?.error || 'Erro ao criar projeto elétrico'),
  })
  ...
  return (
    <div className="mx-auto max-w-xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Novo Projeto Elétrico</h1>
      </div>
      ...
      <Card>
        <form onSubmit={handleSubmit((data) => mutation.mutate(data))} className="space-y-4">
          ...
          <div>
            ...
            <Input {...register('name')} placeholder="Ex: Instalação Elétrica Edifício A" />
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium">Nível de Tensão</label>
              <select {...register('voltageLevel')} className="flex h-10 w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm">
          ...
          <div>
            ...
            <textarea {...register('description')} className="flex w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm min-h-[100px]" placeholder="Descreva o projeto elétrico em detalhe..." />
          </div>
